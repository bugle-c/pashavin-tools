...
| Рекламные расходы (загруженные в Метрику) | Пресет **Рекламные расходы** + при необходимости импорт | [preset_expenses](https://yandex.ru/dev/metrika/ru/stat/presets/preset_expenses), [Data import](https://yandex.ru/dev/metrika/ru/management/openapi/expense) |
...
| Загрузки файлов | Метрики/группировки «Загрузки» (обновления API 2025) | [Metrika API changelog](https://yandex.ru/dev/metrika/ru/changelog) |
...
| Список целей счётчика, id цели | `GET .../management/v1/counter/{id}/goals` | `references/management-api.md` |
...
| Сложные условия | Параметр `filters`, лимиты длины и числа условий | `references/stat-api.md`, [сегментация](https://yandex.ru/dev/metrika/ru/stat/segmentation) |
...
| Как могут спросить | Что делать | Примечание |
|--------------------|------------|------------|
| В Excel / CSV | Суффикс `.csv` в пути `.../data.csv?...` | `references/stat-api.md` |
| Данные «как в интерфейсе» не сходятся | Объяснить семплирование (`accuracy`), таймзону (`timezone`), задержку обработки | Logs vs отчёты — см. FAQ в `references/logs-api.md` |
| Слишком долго / 429 | Квоты, реже дергать `/stat/v1/data/` | `references/quotas.md` |
...
| Как могут спросить | Что делать | Примечание |
|--------------------|------------|------------|
| Выгрузить сырые визиты/хиты | Logs API: create → poll → download → **clean** | `references/logs-api.md` |
| Связать с ClickHouse | Интеграция из доки | `references/logs-api.md` |
| Загрузить расходы на рекламу | Импорт расходов (multipart) | [Data import](https://yandex.ru/dev/metrika/ru/management/openapi/expense) |
| Офлайн-конверсии, CRM, звонки | Импорт офлайн / CRM / calls | [Data import](https://yandex.ru/dev/metrika/ru/management/openapi/expense) |
...
| Как могут спросить | Что делать | Примечание |
|--------------------|------------|------------|
| Какие счётчики есть / id сайта | `GET .../management/v1/counters` | `search_string`, `references/management-api.md` |
