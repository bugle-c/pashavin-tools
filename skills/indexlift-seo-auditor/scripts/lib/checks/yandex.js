...
export function buildYandexFindings(context) {
  ...
  return [
    ...
    (() => {
      const hasJsonLdCountry = Boolean(regionSignals.has_jsonld_country);
      const hasJsonLdArea = Boolean(regionSignals.has_jsonld_area_served);
      const strongVisible =
        regionSignals.region_mentions >= 2 ||
        hasLocalBusiness ||
        pageSnapshot.business_signals.address_mentions > 0;
      const passes = strongVisible || hasJsonLdCountry || hasJsonLdArea;
      const evidenceParts = [];
      if (regionSignals.region_samples?.length) evidenceParts.push(...regionSignals.region_samples);
      if (regionSignals.jsonld_countries?.length) {
        evidenceParts.push(`JSON-LD addressCountry: ${regionSignals.jsonld_countries.join(', ')}`);
      }
      if (regionSignals.jsonld_areas?.length) {
        evidenceParts.push(`JSON-LD areaServed: ${regionSignals.jsonld_areas.slice(0, 4).join(', ')}`);
      }
        ...
        status:
          !looksRussianTargeted
            ...
            : passes
              ? 'PASS'
              : 'WARN',
        value: `${regionSignals.region_mentions || 0} mentions, jsonld country=${hasJsonLdCountry}, areaServed=${hasJsonLdArea}`,
        details:
          !looksRussianTargeted
            ? 'The page does not strongly look like a Russian-targeted page in this heuristic.'
            : passes
              ? 'The page exposes regional or locality signals (visible mentions and/or JSON-LD addressCountry/areaServed) that can help Yandex understand geographic relevance.'
              : 'The page appears Russian-targeted but exposes weak regional signals in visible content, business data, and JSON-LD address/area fields.',
        recommendation:
          !looksRussianTargeted || passes
            ? ''
            : 'Strengthen regional targeting with a clearer city, region, address, or locality signal in visible content and JSON-LD (addressCountry, areaServed).',
