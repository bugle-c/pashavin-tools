...
const DEFINITION_LEAD_PATTERN =
  /\b(это|представляет собой|означает|помогает|позволяет|решает|обеспечивает|включает|содержит|объедин(?:яет|ение)|объединяет в одном|единый сервис|это сервис|сервис для|платформа для|инструмент для|is|are|refers to|defined as|helps|allows|lets|combines|provides|includes|enables|service for|platform for)\b/i;
...
function detectJsonLdRegionSignals(jsonLd) {
  ...
  for (const node of nodes) {
    // PostalAddress with addressCountry
    const addr = node.address;
    const addrs = Array.isArray(addr) ? addr : addr ? [addr] : [];
    for (const a of addrs) {
      if (a && typeof a === 'object') {
        const c = a.addressCountry;
        if (typeof c === 'string') countries.add(c.toUpperCase().trim());
        else if (c && typeof c === 'object' && typeof c.name === 'string') countries.add(c.name.toUpperCase().trim());
        if (typeof a.addressLocality === 'string') areas.add(a.addressLocality.trim());
        if (typeof a.addressRegion === 'string') areas.add(a.addressRegion.trim());
      }
    }
    // areaServed (string | object | array)
    const served = node.areaServed;
    const servedArr = Array.isArray(served) ? served : served ? [served] : [];
    for (const s of servedArr) {
      if (typeof s === 'string') areas.add(s.trim());
      else if (s && typeof s === 'object' && typeof s.name === 'string') areas.add(s.name.trim());
    }
    // ContactPoint.areaServed
    const cps = toArray(node.contactPoint);
    for (const cp of cps) {
      if (cp && typeof cp === 'object') {
        const cpServed = toArray(cp.areaServed);
        for (const s of cpServed) {
          if (typeof s === 'string') areas.add(s.trim());
        }
      }
    }
  ...
  return {
    countries: [...countries],
    areas: [...areas],
    hasJsonLdCountry: countries.size > 0,
    hasJsonLdAreaServed: areas.size > 0,
  };
}
...
export function parseHtmlPage(html, pageUrl, headers = {}) {
  ...
  const jsonLdRegionSignals = detectJsonLdRegionSignals(jsonLd);
